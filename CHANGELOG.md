...
- **КРИТИЧНО**: Добавлен `encoding='utf-8'` ко всем `.write_text()` при создании проекта
- Теперь файлы сохраняются в правильной кодировке и не показывают `����������`
- Все русские комментарии в генерируемых файлах остались (но теперь корректно отображаются)

### Изменения

- Версия обновлена до 3.1.3
- CLI сообщения на английском для совместимости с Windows консолью
- Генерируемые файлы теперь всегда UTF-8
